...
void fence_check()
{
    ...
    if( new_breaches != AC_FENCE_TYPE_NONE ) {
        ...
        if(fence.get_action() != AC_FENCE_ACTION_REPORT_ONLY ) {
            ...
            if (ap.land_complete || (mode_has_manual_throttle(control_mode) && (ap.throttle_zero || !ap.motor_interlock )&& !failsafe.radio && ((fence.get_breaches() & AC_FENCE_TYPE_ALT_MAX)== 0))){
                init_disarm_motors();
            }else{
