...
static void drift_run()
{
    ...
    // if not armed or landed and throttle at zero or motor interlock not enabled, set throttle to zero and exit immediately
    if(!motors.armed() || (ap.land_complete && (ap.throttle_zero || !ap.motor_interlock))) {
